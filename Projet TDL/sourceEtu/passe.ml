...
let analyser_param info =
  ...
  (* Fonction spécifique pour analyser les variables globales (Ast.static) *)
  let analyser_static (Ast.AstPlacement.Static(info, _)) =
    match Tds.info_ast_to_info info with
    | InfoVar (n,_,d,r) -> [(n,(d,r))]
    | _ -> []
  ...
  let analyser (Ast.AstPlacement.Programme (vars,fonctions, (prog,_))) =
    ("main", List.flatten (List.map analyser_static vars) @ List.flatten (List.map (analyser_instruction) prog))::(List.flatten (List.map (analyser_fonction) fonctions))
